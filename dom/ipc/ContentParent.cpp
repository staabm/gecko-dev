...
already_AddRefed<ContentParent> ContentParent::GetUsedBrowserProcess(
  ...
  // Don't try to reuse non-recording processes, as the presence of recording
  // tabs can cause other new tabs to not render. There might be a simple fix
...
bool ContentParent::ShouldKeepProcessAlive() {
  ...
  // Recording content parents cannot be reused and should not be kept alive.
  if (this->IsRecording()) {
    return false;
  }
...
ContentParent::ContentParent(const nsACString& aRemoteType, int32_t aJSPluginID,
  ...
  if (RecordAllContentProcesses()) {
    ...
    mRecordingDispatchAddress = GetRecordReplayDispatchServer();
  }
